...
namespace KCL_rosplan {
    ...
    void RPActionInterface::runActionInterface() {
        ...
        std::vector<rosplan_knowledge_msgs::DomainAssignment>::iterator ait = op.at_start_assign_effects.begin();
        for(; ait!=op.at_start_assign_effects.end(); ait++)
            predicateNames.push_back(ait->LHS.name);

        ait = op.at_end_assign_effects.begin();
        for(; ait!=op.at_end_assign_effects.end(); ait++)
            predicateNames.push_back(ait->LHS.name);
